...
## Project Structure
Java 7 버전을 사용중이며 Spring 4.0.4.RELEASE 버전을 쓰고 있음  

Spring Security ACL을 사용하며 AclCache를 위해 JBoss의 JNDI로 설정 된   
nativeCacche를 SpringEmbeddedCacheManager에 주입하여 사용 중  
...
## 공유 캐시 설정의 필요성
현재 프로젝트에선 Applicaiton 구동 중 권한 정보를 DB에서 읽어와 AclCache에 담는데   
Role에 대한 Privilege가 바뀔 경우를 대비하여 AchCache를 clear하도록 설정했다.  
...
따라서 운영중에 권한이 변경 될 때마다 자주 캐시가 비워져야하는 해당 Application에선 장애가 날 수 있다는 것.

Redis 4.0 이상 버전부터 지원하는 FLUSHALL ASYNC 명령을 사용하면 이러한 문제점을 개선할 수 있다고 한다.  
...
이래서 Spring Boot를 사용하는구나 싶었고 Java나 Spring에 대한 Version 마이그레이션을 두려워하지 않고  
미리미리 진행하는게 어떨까 싶다.  

사실 Java7 -> Java8에 대한 마이그레이션은 크게 문제도 없을 뿐더러 Spring 4.0.4 -> 4.3.26(final)도 마찬가지 아닐까 싶다